    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &MainWindow::next_dir);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_A), this), &QShortcut::activated, this, &MainWindow::prev_dir);
...
void MainWindow::set_args(int argc, char *argv[])
{
  if (argc > 1) {
    ...
    const int dirIdx = m_dirs.addDir(dir.toStdString());
    if (dirIdx >= 0) m_dirs.switchToDir(dirIdx);
    init();
    refresh_folder_list();
    save_dir_state();
  }
...
void MainWindow::restoreLastSession()
{
    ...
    // Try multi-directory state first
    restore_dir_state();
    if (!m_dirs.isEmpty()) {
        const QString lastDir = QString::fromStdString(m_dirs.currentPath());
        const QString boundCls = QString::fromStdString(m_dirs.classesFileOf(m_dirs.currentPath()));
        const int savedProg    = m_dirs.progressOf(m_dirs.currentPath());

        if (!QDir(lastDir).exists()) return;
        if (!get_files(lastDir)) return;

        // Load classes: prefer bound class file, fall back to global objFile
        QString objFile;
        if (!boundCls.isEmpty() && QFile::exists(boundCls))
            objFile = boundCls;
        else
            objFile = s.value("objFile").toString();

        QStringList savedColors;
        if (!objFile.isEmpty() && QFile::exists(objFile)) {
            m_objFilePath = objFile;
            load_label_list_data(objFile);
        ...
        // Restore progress
        if (savedProg >= 0 && savedProg < m_imgList.size())
            goto_img(savedProg);
    ...
    // Fallback: legacy single-directory session
    ...
    // Migrate legacy session into the directory list
    const int legacyIdx = m_dirs.addDir(lastDir.toStdString());
    if (legacyIdx >= 0) m_dirs.switchToDir(legacyIdx);
...
void MainWindow::initSideTabWidget()
{
    ...
    folderTab->setStyleSheet("background: rgb(0,0,17);");
    QVBoxLayout *folderLayout = new QVBoxLayout(folderTab);
    folderLayout->setContentsMargins(6, 8, 6, 6);
    folderLayout->setSpacing(6);

    auto *folderHint = new QLabel("Ctrl+Shift+D / Ctrl+Shift+A\nto switch folders", folderTab);
    folderHint->setStyleSheet("color: rgb(120,180,180); font-size: 11px;");
    folderHint->setWordWrap(true);

    m_folderList = new QListWidget(folderTab);
    m_folderList->setStyleSheet(
        "QListWidget { background: rgb(0,0,17); color: rgb(0,255,255); "
        "border: 1px solid rgb(0,255,255); border-radius: 3px; font-size: 12px; }"
        "QListWidget::item { padding: 5px 8px; border-bottom: 1px solid rgb(0,40,60); }"
        "QListWidget::item:selected { background: rgb(0,60,90); color: rgb(255,255,255); }"
        "QListWidget::item:hover { background: rgb(0,30,50); }");
    connect(m_folderList, &QListWidget::itemClicked, this,
            [this](QListWidgetItem *) { on_folderList_clicked(); });
    ...
    folderLayout->addWidget(m_btnNewFolder);
    ...
    m_sideTabWidget->addTab(folderTab, "\u2302 Folders");
...
void MainWindow::save_dir_state()
{
    QSettings s("YoloLabel", "Session");
    QStringList dirList;
    QVariantMap dirProg, dirCls;
    for (const DirEntry &d : m_dirs.snapshot()) {
        dirList << QString::fromStdString(d.path);
        dirProg.insert(QString::fromStdString(d.path), d.progress);
        if (!d.classesFile.empty())
            dirCls.insert(QString::fromStdString(d.path), QString::fromStdString(d.classesFile));
    }
    s.setValue("dirList", dirList);
    s.setValue("dirProg", dirProg);
    s.setValue("dirCls",  dirCls);
    s.setValue("dirCur",  m_dirs.currentIndex());
}

void MainWindow::restore_dir_state()
{
    QSettings s("YoloLabel", "Session");
    const QStringList dirList = s.value("dirList").toStringList();
    const QVariantMap dirProg = s.value("dirProg").toMap();
    const QVariantMap dirCls  = s.value("dirCls").toMap();
    const int dirCur          = s.value("dirCur", 0).toInt();
    ...
    for (const QString &p : dirList) {
        DirEntry e;
        e.path = p.toStdString();
        e.progress = dirProg.value(p, 0).toInt();
        e.classesFile = dirCls.value(p).toString().toStdString();
        entries.push_back(e);
    }
...
{
    if (!m_folderList) return;
    m_folderList->blockSignals(true);
    m_folderList->clear();

    const auto dirs = m_dirs.snapshot();
    for (int i = 0; i < static_cast<int>(dirs.size()); ++i) {
        const DirEntry &d = dirs[i];
        QString name = QFileInfo(QString::fromStdString(d.path)).fileName();
        if (name.isEmpty()) name = QString::fromStdString(d.path);
        int prog = d.progress;
        int total = (m_imgDir == QString::fromStdString(d.path)) ? m_imgList.size() : -1;
        QString text = (total > 0)
            ? QString("%1  (%2/%3)").arg(name).arg(prog + 1).arg(total)
            : name;
        auto *item = new QListWidgetItem(text, m_folderList);
        item->setData(Qt::UserRole, i);
        if (i == m_dirs.currentIndex())
            item->setSelected(true);
    }
    m_folderList->blockSignals(false);
}

void MainWindow::on_folderList_clicked()
{
    auto *item = m_folderList->currentItem();
    if (!item) return;
    switch_to_dir(item->data(Qt::UserRole).toInt());
}

void MainWindow::switch_to_dir(int index)
{
    if (m_dirs.isEmpty()) return;
    if (m_cloudLabeler && m_cloudLabeler->isBusy()) return;

    const int n = m_dirs.size();
    if (index < 0 || index >= n) return;

    // Same directory — just restore progress
    if (index == m_dirs.currentIndex()) {
        int prog = m_dirs.progressOf(m_dirs.currentPath());
        if (prog >= 0 && prog < m_imgList.size())
            goto_img(prog);
        return;
    }

    // Save current progress before switching
    m_dirs.setProgress(m_imgDir.toStdString(), m_imgIndex);
    if (ui->label_image->isOpened()) save_label_data();

    // Switch state
    if (!m_dirs.switchToDir(index)) return;
    const QString newDir = QString::fromStdString(m_dirs.currentPath());
    const QString newCls = QString::fromStdString(m_dirs.classesFileOf(m_dirs.currentPath()));
    const int savedProg  = m_dirs.progressOf(m_dirs.currentPath());

    // Load classes for the target dir if bound
    if (!newCls.isEmpty()) {
        m_objFilePath = newCls;
        load_label_list_data(newCls);
    }
    // If no bound classes and global list is empty, keep current classes

    // Load images from the new directory
    if (!get_files(newDir)) {
        statusBar()->showMessage("Directory is empty or unreadable: " + newDir, 4000);
        return;
    }
    ...
    statusBar()->showMessage(
        QString("Switched to: %1  (%2/%3)")
            .arg(QFileInfo(newDir).fileName())
            .arg(m_dirs.currentIndex() + 1).arg(n), 3000);
}

void MainWindow::next_dir()
{
    int idx = m_dirs.nextDir();
    if (idx >= 0) switch_to_dir(idx);
}

void MainWindow::prev_dir()
{
    int idx = m_dirs.prevDir();
    if (idx >= 0) switch_to_dir(idx);
}
